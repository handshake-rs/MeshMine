...
| Native mining node | lean `hsrd` workspace with product bloat removed, exact HNS network/genesis parameters, derived unsigned 256-bit chainwork, HNS difficulty/timestamp admission, database/network binding, durable monotonic tip generations, staged lag-recoverable events, immutable prepared jobs, direct solved-candidate admission, exact body validation, and an in-process snapshot/body/assignment-to-gateway bridge with fail-closed restart binding; complete HNS consensus, Urkel, live P2P, atomic multi-block reorgs, templates/relay, and parity evidence remain open |
