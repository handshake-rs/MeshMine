...
HNS MeshMine is a research implementation of the no-hard-fork, independently templated Handshake mining overlay specified by [MM-0001 Core v2](MeshMine.md). It preserves ordinary HNS consensus. The Core/operator path now uses authenticated native [`hsrd`](hsrd/README.md) state as its sole runtime parent source; `hsd` is pinned only as an offline fixture and differential oracle. `hsrd` remains pre-authority, so Core fails closed until historical, invalid-corpus, reorg, state-root, and deployment readiness gates pass.
